...
summary = pd.read_csv("../../../../../Results/Inside_Contact/Resnet50/Pruned_Mask_Parameters/summary.csv")
...
for _, grp in configs:
    model_id = "/".join(grp["0_exp_dir"].iloc[0].split("/")[-4:-2])
    print(model_id)
    ...
    config_name = model_id.split("/")[-1] + "_T" + str(task)
    print(config_name)
    ...
    config["pretrained_weights"]["mlp"] = os.path.join(config["pretrained_weights"]["mlp"], model_id + "_mlp.pt")
    config["lr_list"] = [lr]
    config["l0_init_list"] = [l0_init]
    config["l0_stage_list"] = [stages]
