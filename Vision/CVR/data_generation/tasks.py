...
def task_shape(condition='xysc'): # condition='xysc'
    ...
    shape = Shape()
    shape_odd = Shape()
    shape = [[shape.clone()] for i in range(n_samples-1)] + [[shape_odd]]
...
def task_pos(condition='xyc'):
    ...
    xy = np.random.rand(2) * (1-size) + size/2
    if 'xy' in condition:
...
def task_count(condition='xysid', condition_arr='xysid'):
    ...
    if 'id' in condition_arr and 'id' in condition:
        id_idx = np.zeros([n_samples, max_n_objs])
    elif 'id' in condition_arr and 'id' not in condition:
        id_idx = np.stack([np.arange(n_samples)]*max_n_objs, axis=1)
    elif 'id' not in condition_arr and 'id' in condition:
        id_idx = np.stack([np.arange(max_n_objs)]*n_samples, axis=0)
    elif 'id' not in condition_arr and 'id' not in condition:
        id_idx = np.arange(max_n_objs*n_samples).reshape([n_samples, max_n_objs])

    if 's' in condition_arr and 's' in condition:
        s_idx = np.zeros([n_samples, max_n_objs])
    elif 's' in condition_arr and 's' not in condition:
        s_idx = np.stack([np.arange(n_samples)]*max_n_objs, axis=1)
    elif 's' not in condition_arr and 's' in condition:
        s_idx = np.stack([np.arange(max_n_objs)]*n_samples, axis=0)
    elif 's' not in condition_arr and 's' not in condition:
...
def task_pos_pos_1(condition='xyc'):
    ...
    if 'xy' in condition:
        ...
        xy_sc = np.ones([n_samples, 2]) * np.random.rand(1,2) * (1 - size_sc) + size_sc/2
...
def task_pos_pos_2(condition='xyc'):
    ...
    while n_unique_xy_sc_left>0:
        xy_sc = np.random.rand(n_samples_over,2,2) * (1 - size_sc) + size_sc/2
        valid = (np.abs(xy_sc[:,0,:] - xy_sc[:,1,:]) - size_sc > 0).any(1)
        if valid.sum()>0:
...
def task_pos_pos_3(condition='s_id_c'):
    ...
    # size_d = np.random.rand(n_samples)*(max_s_d - min_s_d) + min_s_d
    # size = np.cumsum(size_d)[np.random.permutation(n_samples)] + min_size
    if 's' in condition:
    ...
    # x1 = np.random.rand(n_samples) * (internal_frame / 2 - (size[:,0]+size[:,1])/2)
    # x2 = np.random.rand(n_samples) * (internal_frame / 2 - (size[:,0]+size[:,1])/2)
...
def task_pos_shape_2(condition='c'):
    ...
    xy = np.stack([xy + links_[:,0,:]*size/2, xy + links_[:,1,:]*size/2], 1)

    xy = xy.reshape(n_objects,2)
...
def task_pos_rot_3(condition='xyc'):
...
def task_pos_flip_1(condition='xyc'):
...
def task_pos_flip_2(condition='xyc'):
...
def task_pos_col_1(condition='xyscid_'):
    ...
    if 's' in condition:
        s = (np.random.rand(n_samples, 2) * 0.4 + 0.6) * np.ones([n_samples, 2])
    else:
    ...
    for i in range(n_samples):
        ...
        for _ in range(max_attempts):
            ...
            if axis == 'x':
                ...
                change_xy = xy[np.logical_and(xy[:,1]>0.45, xy[:,1]<0.55), 0]
                change_xy = change_xy + np.random.choice([-1,1], size=change_xy.shape[0])*0.2
                xy[np.logical_and(xy[:,1]>0.45, xy[:,1]<0.55), 1] = change_xy

                # xy[np.logical_and(xy[:,1]>0.4, xy[:,1]<0.6), 1] = xy[:,1] + np.random.choice([-1,1], size=n_samples_over*n_objects)*0.3
...
def task_pos_inside_4(condition='s_id_c', variable='x'):
    ...
    # x1 = np.random.rand(n_samples) * (1 - size[:,0] - size[:,1]*2) + (size[:,0]/2)
    # x2 = np.random.rand(n_samples) * (1-size[:,1] - x1 - size[:,0]/2) + x1 + size[:,0]/2 + size[:,1]/2
    # x = np.stack([x1, x2], 1)
...
def task_pos_count_4(condition='xyscid', cue='xid'):
    ...
    if 'id' in cue:
        id_idx = [[0]*n_objects_1[i] + [1]*n_objects_2[i] + [2]*n_objects_3[i] for i in range(n_samples)]
        n_id_idx = 3
    else:
        if 'id' in condition:
            id_idx = [[0]*n_objects[i] for i in range(n_samples)]
            n_id_idx = 1
        elif 'id' not in condition:
            id_idx = []
            j = 0
            for i in range(n_samples):
                id_idx += list(range(j, j+n_objects[i]))
                j = j+n_objects[i]
            n_id_idx = j

    unique_shapes = []
    for i in range(n_id_idx):
        s = Shape()
    ...
    for i in range(n_samples):
        ys_ = []
        for n_objs in [n_objects_1, n_objects_2, n_objects_3]:
            dists_y = np.random.rand(n_objs[i])# * (1 - size[i]*n_objs[i])
            dists_y = dists_y/dists_y.sum() * (1 - size[i]*n_objs[i])
...
def task_size_size_1(condition='xyc', reference='id'):
    ...
    if 'x' in reference:
        dists = np.random.rand(n_samples, 2)
        dists = dists / dists.sum(1)[:, None] * (1 - size.sum(1)[:, None])
        ...
        dists = np.random.rand(1, 2)
        dists = dists / dists.max(0).sum(0) * (1 - size.max(0)[None, :])
        dists[:,0] = dists[:,0] * np.random.rand(1)
        dists = dists + size[:,0:1]
        x = np.cumsum(dists, 1)
        x = x - size[:,0:1]/2

        x = x[0:1]
        x = x * np.ones(n_samples)[:,None]
    ...
    if 'y' in reference:
        ...
        np.apply_along_axis(np.random.shuffle, 1, y)
...
def task_size_size_2(condition='xyc'):
...
def task_size_contact(condition='xys'):
    ...
    for i in range(n_samples):
        ...
        # xy01 = np.random.rand(2) * (1-clump_size1) + clump_size1/2
        # xy02 = np.random.rand(2) * (1-clump_size2) + clump_size2/2
...
def task_shape_contact_2(condition=''):
    ...
    # dirs = [np.random.choice([0,1,2,3], size=2, replace=False) for i in range(n_samples)]
    # cond_reg = [0]
    # cond_odd = [0]
    # # cond_odd = [1,2]
    # possibilities = np.random.choice(cond_reg, size=n_samples)
    # possibilities[-1] = np.random.choice(cond_odd)
...
def task_rot_flip_1(condition='xyc'):
...
task_rot_contact_2 = lambda: task_shape_contact_5('r')
...
def task_flip_inside_2(condition='xyscid_hsv'):
    n_samples = 4
    n_objects = np.random.randint(2,4)
    n_objects_in = np.random.randint(1, n_objects, size=n_samples)
    max_attempts = 20
...
task_flip_contact_2 = lambda: task_shape_contact_5('f')
...
def task_color_color_2(condition='nxyscid_hsv', variable='h'):
    ...
    if variable == 'v':
        ...
        v = (np.random.rand(n_samples, n_objects_max) * 0.3 + 0.7)
    else:
    ...
    # if 's' in condition:
    #     size = np.random.rand(n_samples, n_objects_max) * (max_size-min_size) + min_size
    # else:
    #     size = np.random.rand() * (max_size-min_size) + min_size
...
def task_color_inside_2(condition='xyscid_hsv'):
    ...
    if 's' in condition:
        s = (np.random.rand(n_samples+1, n_objects) * 0.4 + 0.6)
    else:
        s = np.random.rand() * 0.4 + 0.6
...
def task_color_count_1(condition='xyidc'):
    ...
    # else:
    #     h = np.random.rand()
    #     h = h * np.ones([n_samples, n_groups])
...
def task_color_count_2(condition='xyidc'):
    ...
    # else:
    #     h = np.random.rand()
    #     h = h * np.ones([n_samples, n_objects])
    ...
    for i in range(n_samples):
        ...
        for _ in range(max_attempts):
            ...
            # a, b(c)
                ...
                # bbs_ab = bbs_ab * np.array([size_a, size_b])[:,None]
                ...
                # xy.append(np.array([xy_ab[0], xy_ab[1], xy_c]))
                # sizes.append([size_a, size_b, size_c])
                # shapes.append([sa, sb, sc])
            ...
            # a(b),c
                ...
                # ca = sa.get_contour()
                # cc = sc.get_contour()
                # bbs_ac = np.stack([ca.max(0) - ca.min(0), cc.max(0) - cc.min(0)], 0)
                # bbs_ac = bbs_ac * np.array([size_a, size_c])[:,None]
                ...
                # while not (np.abs(xy_ac[:,0] - xy_ac[:,1]) - (size_a + size_c)/2 > 0).any(1).any(0):
                #     xy_ac = np.random.rand(10, 2, 2) * range_ac[None,:,None] + starting_ac[None,:,None]
                ...
                # xy.append(np.array([xy_ac[0], xy_b, xy_ac[1]]))
                # sizes.append([size_a, size_b, size_c])
                # shapes.append([sa, sb, sc])
            ...
            # a,b,c
                ...
                # no_overlap = dists.any(3)[:, np.triu_indices(3, k=1)].all(1)
...
def task_count_count(condition='xyscid', condition_arr='xysid'):
...
def task_color_sym_1(condition='xyc'):
    ...
    if 's' in condition:
        s = (np.random.rand(n_samples, n_objects) * 0.4 + 0.6) * np.ones([n_samples, n_objects])
    else:
        s = np.random.rand() * 0.4 + 0.6
        s = s * np.ones([n_samples, n_objects])
...
def task_color_sym_2(condition='xyc'):
...
TASKS=[
    ...
    ["sn_task_1_contact", sn_task_1_contact, "Same as task contact, with constraints on color"],
    ["sn_task_1_contact_inside_both_ooo", partial(sn_task_1_contact_inside, odd_one_out="no_contact_inside"), "Same as contact inside, with the odd one out always being outside and not in contact."],
    ...
    ["sn_task_2_inside", sn_task_2_inside, "Same as inside count with different rule assignment. + all nested, - some nonnested"],
    ["sn_task_2_count", sn_task_2_count, "Same as inside count with different rule assignment. + same # total shapes, - diff count"],
    ["sn_task_2_inside_count_both_ooo", partial(sn_task_2_inside_count, odd_one_out="no_count_inside"), "Same as inside count, with the odd one out always being outside and different number of shapes."],
    ["sn_task_2_inside_count_ablate_inside", partial(sn_task_2_inside_count, odd_one_out="no_count"), "Same as inside count, with the odd one out always having a different number of shapes. Ablating inside should still achieve high perf."],
    ["sn_task_2_inside_count_ablate_count", partial(sn_task_2_inside_count, odd_one_out="no_inside"), "Same as inside count, with the odd one out always being outside. Ablating count should still achieve high perf."],
    ["sn_task_3_contact_count", sn_task_3_contact_count, "3 images contain N objects, all in contact. OOO either has a diff number of shapes and/or one not in contact"],
    ["sn_task_3_contact", sn_task_3_contact, "3 images contain objects all in contact. OOO either has one not in contact"],
    ["sn_task_3_count", sn_task_3_count, "3 images contain N objects. OOO either has one not with != N objects"],
    ["sn_task_3_contact_count_both_ooo", partial(sn_task_3_contact_count, odd_one_out="no_contact_count"), "Same as contact count, but the OOO examples always fail both conditions"],
    ["sn_task_3_contact_count_ablate_contact", partial(sn_task_3_contact_count, odd_one_out="no_count"), "Same as contact count, with the odd one out always having a different number of shapes. Ablating contact should still achieve high perf."],
    ["sn_task_3_contact_count_ablate_count", partial(sn_task_3_contact_count, odd_one_out="no_contact"), "Same as contcct count, with the odd one out always having one shape not in contact. Ablating count should still achieve high perf."],
]
