...
for base_model in base_models:
    pruned_model = os.listdir(os.path.join(config["base_dir"], base_model))[0]
    model_path = os.path.join(config["base_dir"], base_model, pruned_model)
    mask_training_tasks = os.listdir(model_path)
    for mask_task in mask_training_tasks:
        mask_configurations = os.listdir(os.path.join(model_path, mask_task))
        ...
        for mask_configuration in mask_configurations:
            try:
                data.append(pd.read_csv(os.path.join(model_path, mask_task, mask_configuration, "results.csv")))
            except:
                print(base_model)
