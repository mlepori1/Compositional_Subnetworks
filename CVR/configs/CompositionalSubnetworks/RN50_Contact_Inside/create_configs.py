...
with open("RN50_base_config.yaml", 'r') as stream:
    base = yaml.load(stream, Loader=yaml.FullLoader)

os.makedirs("Experiment_Configs", exist_ok=True)

l0_inits = [0.0, -0.01, -0.02, -0.03, -0.05] # from section 5.3 of continuous sparsification
train_tasks = ['104', '105'] # Train insideness and contact subnets

pretrained_models_dir = "/gpfs/data/epavlick/mlepori/projects/Compositional_Subnetworks/Models/resnet50/ConIn_Models"
