...
class TemperatureCallback(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        # There is only a "backbone" for the NLP experiments, a BERT model
...
def cli_main():
    ...
    parser.add_argument('--model', type=str, default='BERT', help='Model to use')
    parser.add_argument('--dataset', type=str, default='SyntaxDatamodule', help='dataset to use for training')
    ...
    base_l0_lambda = copy.deepcopy(args.l0_lambda)
    ...
    for seed in args.seed_list:
       ...
       #             for l0_init in args.l0_init_list:
        ...
        # Reset back to base pretrained wieghts and LM_init decision
        args.pretrained_weights = copy.deepcopy(base_pretrained_weights)
        args.LM_init = base_LM_init
        args.l0_lambda = base_l0_lambda

        for iteration in range(args.prune_iterations):
            # Increment model ID for next training
            model_id += 1

            # Reset configuration of training masks and weights
            args.train_masks = copy.deepcopy(base_train_masks)
            args.train_weights = copy.deepcopy(base_train_weights)

            args.task = args.train_task # Prespecified, same for next few comments
            ...
            if args.use_last == True:
                # Disable loading best model for experiments
            ...
            # Save best model weights for testing
            trained_weights = {
            ...
            # Save pruned model weights for further training
            best_model.prune()

            # Print some pruning stats
            print("$$$ ITERATION: " + str(iteration))
            pruned = 0
            total = 0
            for layer in best_model.backbone.modules():
                if hasattr(layer, "mask_weight"):
                    pruned += torch.sum((layer.weight == 0).int())

            print("^^^ Total Pruned: " + str(pruned))

            pruned_weights = {
                "backbone": os.path.join(args.exp_dir, "seed_" + str(seed) + "iter_" + str(iteration) + '_backbone_pruned.pt'),
            }

            torch.save(best_model.backbone.state_dict(), pruned_weights["backbone"])
            ...
            output_dict = {
                    ...
                    '2_prune_iter': iteration,
                    '3_backbone': args.backbone,
                    '3_batch_size': args.batch_size,
                    '3_lr': args.lr,
                    '3_l0_init': args.l0_init,
                    '3_l0_stages': args.l0_stages
                }
            ...
            # using the weights we just trained (unpruned version)
            args.pretrained_weights = trained_weights
            args.LM_init = False # LM init will stay false until the next seed
            ...
            for ablation in args.ablation_strategies:
                ...
                for task in args.test_tasks:
                    ...
                    if args.evaluation_type == "test":
                        ...
                        # Maintain the same reporitng logic during test and hparam search, just different data
            ...
            # Set pretrained weights equal to the pruned weights
            # Next iteration will continue pruning from the current 
            # pruned model iteration
            args.pretrained_weights = pruned_weights

            # anneal lambda
            args.l0_lambda = args.l0_lambda * 10

        # Get rid of trained models after full rounds of pruning
