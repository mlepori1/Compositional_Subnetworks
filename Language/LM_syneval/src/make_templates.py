...
class MakeAgreementTemplate():
    ...
    def make_variable_sents(self, preterms, match, vary, case_name):
        all_sents = {}
        base_sent = [self.terminals[p] for p in preterms]

        # Case_name now defines whether its grammatical singular or grammatical plural
        if "singular" in case_name:
            prefix = "sing"
        elif "plural in case_name":
            prefix = "plur"
        ...
        all_sents[self.get_case_name(preterms, match, vary, opt=prefix, v_opt='sing')] = [s_grammatical, s_ungrammatical]
        if len(vary) > 0:
            all_sents[self.get_case_name(preterms, match, vary, opt=prefix, v_opt='plur')] = [p_grammatical, p_ungrammatical]
...
class MakeTestCase():
    ...
    def get_rules(self):
        sent_templates = {}
        base_test_case = self.test_case[self.test_case.index("_") + 1 :] # Get rid of "singular_" or "plural_"
        preterminals, templates = self.template.rules[base_test_case]
        if templates is not None:
            sents = self.template.make_variable_sents(preterminals, templates['match'], templates['vary'], self.test_case)
            for k in sents.keys():
...
def main():
    ...
    for case in agrmt_test_cases:
        ...
        print(np.sum([len(sents.sent_templates[temp]) * 2 for temp in sents.sent_templates]))
